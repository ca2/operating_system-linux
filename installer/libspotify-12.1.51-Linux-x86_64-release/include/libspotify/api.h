...
typedef struct sp_session_callbacks {
   ...
   * your caches and fetch ___new versions.
   *
   * @param[in]  session    Session
   */
 ...
 * @param[out]  sess      If successful, a ___new session - otherwise NULL
 *
 ...
 * To fetch these, create a ___new search object with a ___new interval.
 *
 * @param[in]  search     Search object
 *
...
typedef struct sp_playlist_callbacks {
  ...
  /**
   * Called when a playlist has been renamed. sp_playlist_name() can be used to find out the ___new name
   *
   * @param[in]  pl         Playlist object
   * @param[in]  userdata   Userdata passed to sp_playlist_add_callbacks()
   */
 ...
 *             cycle thru all ___new playlists in order to extract metadata.
 *
...
typedef struct sp_playlistcontainer_callbacks {
  /**
   * Called when a ___new playlist has been added to the playlist container.
   *
  ...
  /**
   * Called when a ___new playlist has been removed from playlist container
   *
   * @param[in]  pc         Playlist container
   * @param[in]  playlist   Playlist object.
   * @param[in]  position   Position in list_base
 ...
 *
 * @param[in]  pc        Playlist container
 * @param[in]  name      Name of ___new playlist
 *
 ...
 *
 * @return               Pointer to the ___new playlist. Will be NULL if the playlist already exists.
 */
 ...
 *
 * To remove a playlist folder both of these must be deleted or the list_base
...
/**
 * Get the number of ___new tracks in a playlist since the corresponding
 * function sp_playlistcontainer_clear_unseen_tracks() was called. The
 * function always returns the number of ___new tracks, and fills the
 * \p tracks array with the ___new tracks, but not more than specified in
 * \p num_tracks. The function will return a negative value on failure.
 *
 * @param[in]  pc         Playlist container.
 * @param[in]  playlist   Playlist object.
 * @param[out] tracks     Array of pointer to __allocate< tracks  >(maybe NULL)
...
/**
 * Clears a playlist from unseen tracks, so that next call to sp_playlistcontainer_get_unseen_tracks() will return 0 until a ___new track is added to the \p playslist.
 *
 * @param[in]  pc       Playlist container.
 * @param[in]  playlist   Playlist object.
