...
typedef struct sp_session_callbacks {
   ...
   * your caches and fetch __new versions.
   *
   * @param[in]  session    Session
   */
 ...
 * @param[out]  sess      If successful, a __new session - otherwise NULL
 *
 ...
 * To fetch these, create a __new search object with a __new interval.
 *
 * @param[in]  search     Search object
 *
...
typedef struct sp_playlist_callbacks {
  ...
  /**
   * Called when a playlist has been renamed. sp_playlist_name() can be used to find out the __new name
   *
   * @param[in]  pl         Playlist object
   * @param[in]  userdata   Userdata passed to sp_playlist_add_callbacks()
   */
 ...
 *             cycle thru all __new playlists in order to extract metadata.
 *
...
typedef struct sp_playlistcontainer_callbacks {
  /**
   * Called when a __new playlist has been added to the playlist container.
   *
  ...
  /**
   * Called when a __new playlist has been removed from playlist container
   *
   * @param[in]  pc         Playlist container
   * @param[in]  playlist   Playlist object.
   * @param[in]  position   Position in list
 ...
 *
 * @param[in]  pc        Playlist container
 * @param[in]  name      Name of __new playlist
 *
 * @return               Pointer to the __new playlist. Can be NULL if the operation fails.
 */
 ...
 *
 * @return               Pointer to the __new playlist. Will be NULL if the playlist already exists.
 */
...
/**
 * Get the number of __new tracks in a playlist since the corresponding
 * function sp_playlistcontainer_clear_unseen_tracks() was called. The
 * function always returns the number of __new tracks, and fills the
 * \p tracks array with the __new tracks, but not more than specified in
...
/**
 * Clears a playlist from unseen tracks, so that next call to sp_playlistcontainer_get_unseen_tracks() will return 0 until a __new track is added to the \p playslist.
 *
 * @param[in]  pc       Playlist container.
 * @param[in]  playlist   Playlist object.
