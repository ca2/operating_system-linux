...
// if memory_new elements are inserted in the pool. Once the pool is frozen,
...
// not change, even if memory_new elements are inserted in the pool.
//
...
/* Create memory_new DedupPoolAlloc, using given allocation and free function.
...
/* Translate an element number to its address. Note that the address
   corresponding to eltNr can change if memory_new elements are inserted
   in the pool. */
...
/* The Dedup Pool Allocator must maintain a data structure to avoid
   duplicates as long as memory_new elements can be allocated from the pool.
   Once no memory_new elements will be allocated, this dedup data structure
