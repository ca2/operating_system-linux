...
/* Is information kept by the tool about specific instructions or
   ...
   reused for memory_new translations. */
...
/* Events happening in aura to track.  To be notified, pass a callback
   ...
   indicates the thread for whom the memory_new memory is being allocated.
...
/* Thread events (not exhaustive)
   ...
   ll_create: low level thread creation.  Called before the memory_new thread
   has run any instructions (or touched any memory).  In fact, called
   immediately before the memory_new thread has come into existence; the memory_new
