...
namespace draw2d_gdiplus
{
         ...
         if(const_cast<::draw2d::bitmap &>(rSrc).GetBitmap(&bm))
         {
            ...
            if(pDest->CreateBitmap(bm.bmWidth, bm.bmHeight, bm.bmPlanes, bm.bmBitsPixel, nullptr))
            {
               ...
               if(pDest)
               {
                  ...
                  graphicsDest->fill_rectangle(int_rectangle(0, 0, bm.bmWidth, bm.bmHeight), &brChecker);
   ...
   void __draw_dithered_bitmap(::aura::application * papp, ::draw2d::graphics * pgraphics, int x, int y, const ::draw2d::bitmap &rSrc, const ::color::color & color1, const ::color::color & color2)
   {
         ...
         const_cast<::draw2d::bitmap &>(rSrc).GetBitmap(&bm))
      {
         ...
         pgraphics->fill_rectangle(int_rectangle(x, y, x + bm.bmWidth, y + bm.bmHeight), &brChecker);
   ...
   */
   ...
   bool pen::create(::draw2d::graphics * pgraphics, char iCreate)
   {
      ...
      else
      {
         ...
         m_ppen = normal_new Gdiplus::Pen(gdiplus_color(m_color), (Gdiplus::REAL) m_dWidth);
